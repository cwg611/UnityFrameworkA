...
public class EmojiBuilder : EditorWindow
{
    ...
    private string m_inputPath = "Assets/HotUpdateResources/UI/Emoji/Input/";
    private string m_outputPath = "Assets/HotUpdateResources/UI/Emoji/Output/";
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label("注意：每个表情图片尺寸需要统一，\n图片命名规范： [纯字母_数字.png]，例：a.png, b_0.png，b_1.png");
        m_emojiSize = EditorGUILayout.IntField("单个表情图尺寸", m_emojiSize);
        m_emojiSuffix = EditorGUILayout.IntField("生成的图集文件后缀", m_emojiSuffix);
    ...
    public void GenerateEmojiAtlas()
    {
        ...
        foreach (string key in sourceDic.Keys)
        {
            for (int index = 0; index < sourceDic[key]; index++)
            {
                ...
                for (int i = 0; i < m_emojiSize; i++)
                {
                    ...
                    {
                        newTex.SetPixel(x + i, y + j, colors[i + j * m_emojiSize > colors.Length - 1 ? colors.Length - 1 : i + j * m_emojiSize]);
                    }
                }
        ...
        string outputfile1 = m_outputPath + "emoji_tex" + m_emojiSuffix + ".png";
        File.WriteAllBytes(outputfile1, bytes1);

        byte[] bytes2 = dataTex.EncodeToPNG();
        string outputfile2 = m_outputPath + "emoji_data" + m_emojiSuffix + ".png";
        File.WriteAllBytes(outputfile2, bytes2);

        string emojiOutputPath = "Assets/HotUpdateResources/Other/";
        using (StreamWriter sw = new StreamWriter(emojiOutputPath + "emoji" + m_emojiSuffix + ".txt", false))
        {
        ...
        //File.Copy(m_outputPath + "emoji.txt", "Assets/HotUpdateResources/Other/emoji.txt", true);
    ...
    {
        TextureImporter emojiTex = AssetImporter.GetAtPath(m_outputPath + "emoji_tex" + m_emojiSuffix + ".png") as TextureImporter;
        ...
        TextureImporter emojiData = AssetImporter.GetAtPath(m_outputPath + "emoji_data" + m_emojiSuffix + ".png") as TextureImporter;
